...
int main(int argc, char *argv[]) {
    ...
    for(int k = 0; k < n; k++){
        ...
        #pragma omp parallel reduction(max:maxi)
        for(int i = k; i < n; i++){
            if(fabs(a[i][k])> maxi){
                maxi = fabs(a[i][k]);
        ...
        double temp_l=0.0;
        #pragma omp parallel for private(temp_l) if(k>500)
        for(int i = 0; i < k; i++){
            temp_l = l[k][i];
            l[k][i] = l[kdash][i];
            l[kdash][i] = temp_l;
        ...
        //upating A[][]
        #pragma omp parallel for private(j,val,temps)
        for(int i = k+1; i < n; i++){
            val=l[i][k] ;
    ...
    // // verify that PA = LU(for correctness of algorithm)
    ...
    cout<<"OMP Time: "<< fixed << time_spent<<endl;
